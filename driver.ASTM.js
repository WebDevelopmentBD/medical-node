...
exports.start = function(port, name, queueFn, queryFn){
  ipuQuery = queryFn;
...
function boot(port, name, queueFn){
	return net.createServer(socket => handleClient(socket, name, queueFn));
	/*
  try {
    const server = net.createServer(socket => handleClient(socket, name, queueFn));
