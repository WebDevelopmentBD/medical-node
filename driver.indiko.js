...
/**
 * Thermo Fisher Indiko / Gallery ASTM E1394 Driver — Pure Receive Broker
 *
 ...
 *   CS  = 2-char hex checksum (not validated — trusted internal device)
 *
 * Wire sequence (instrument -> broker):
 *   <ENQ> -> <ACK>
 *   <STX>1H|...<CR><ETX>CS<CR><LF> -> <ACK>
 *   <STX>2P|...<CR><ETX>CS<CR><LF> -> <ACK>
 *   ...
 *   <EOT> -> <ACK>   <- flush records to remote server
 *
 * Frame numbers cycle: 1,2,3,4,5,6,7,0,1,2,...
...
function handleClient(socket, name, queueFn) {
  ...
  socket.on('data', function(data) {
    try {
      ...
      /* 5. STX frames: parse, extract record, ACK each */
      while (true) {
