...
/**
 * Lifotronic GH-900 Plus ASTM E1381 Driver — Pure Receive Broker
 *
 * Protocol: ASTM-1381 (ARKRAY standard) per GH900Plus-ASTM-Communication.pdf
 *
 * Frame format (each record = 1 frame):
 *   <STX> FN <DATA><CR> <ETX|ETB> <CSH><CSL> <CR> <LF>
 *   FN  = frame number ASCII '1'-'7','0' (starts at 1, wraps to 0 after 7)
 *   ETX = end frame  |  ETB = intermediate frame
 *   CS  = 2-char hex, byte-sum FN..ETX/ETB mod 256
 *
 * Wire sequence (instrument → broker):
 *   <ENQ> → <ACK>
 *   <STX>1H|...<CR><ETB>CS<CR><LF> → <ACK>
 *   <STX>2P|...<CR><ETB>CS<CR><LF> → <ACK>
 *   ...
 *   <STX>0L|1|N<CR><ETB>CS<CR><LF> → <ACK>
 *   <EOT> → <ACK>   ← flush records to remote server
 *
 * Note: Query messages are NOT supported by this device.
 */
